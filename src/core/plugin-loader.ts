...
import {
  findResource, findOperation,
  type Plugin, type PluginDescriptor, type CredentialSpec, type Resource,
  type HttpMethod, type ParameterType, type ParameterLocation, type OutputFormat,
...
const VALID_HTTP_METHODS = new Set<HttpMethod>(["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"]);
const VALID_PARAM_TYPES = new Set<ParameterType>(["string", "number", "boolean", "object", "array"]);
const VALID_PARAM_LOCATIONS = new Set<ParameterLocation>(["query", "path", "header", "body", "cookie"]);
const VALID_CRED_TYPES = new Set<CredentialSpec["type"]>(["api_key", "oauth2", "bearer", "basic", "custom"]);
...
function validateEnum<T extends string>(value: unknown, valid: Set<T>, fallback: T, label: string): T {
...
function validateOutputSpec(raw: unknown): { format: OutputFormat; schema?: Record<string, unknown>; description?: string } {
  ...
  return {
    ...
    schema: typeof obj.schema === "object" && obj.schema !== null ? obj.schema as Record<string, unknown> : undefined,
...
async function loadYamlPlugin(filePath: string): Promise<Plugin> {
  ...
  const descriptor: PluginDescriptor = {
    ...
    credentials: (manifest.credentials ?? []).map((c) => ({
      ...
      fields: Array.isArray(c.fields) ? c.fields.map((f: Record<string, unknown>) => ({
        name: String(f.name ?? ""),
    ...
    resources: (manifest.resources ?? []).map((r) => ({
      ...
      operations: (r.operations ?? []).map((op) => ({
        ...
        path: String(op.path ?? "/"),
        parameters: Array.isArray(op.parameters) ? op.parameters.map((p: Record<string, unknown>) => ({
          name: String(p.name ?? ""),
          ...
          default: p.default,
          location: validateEnum(p.location, VALID_PARAM_LOCATIONS, "query", "parameter location"),
...
export async function loadPluginsFromDir(
  ...
  try {
    ...
    console.error(`[nathan] Warning: Cannot read plugin directory ${dirPath}: ${err instanceof Error ? err.message : String(err)}`);
    return;
  }
